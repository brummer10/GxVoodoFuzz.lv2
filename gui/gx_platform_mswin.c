...
void gx_gui_create_window_and_surface(gx_voodooUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_voodooUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_voodooUI *ui) {
	...
	UnregisterClass(TEXT("gx_voodooUIClass"), NULL);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_voodooUI *ui = (gx_voodooUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
