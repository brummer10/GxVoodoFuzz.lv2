...
static void resize_event(gx_voodooUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_voodooUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_voodooUI* ui = (gx_voodooUI*)malloc(sizeof(gx_voodooUI));
	...
	ui->width = ui->init_width -140 + (75 * CONTROLS);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/(ui->init_width-140 + (75 * CONTROLS));
...
static void cleanup(LV2UI_Handle handle) {
	gx_voodooUI* ui = (gx_voodooUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_voodooUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_voodooUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_voodooUI *ui, gx_controller* controller) {
...
static void _expose(gx_voodooUI *ui) {
	static const char* plug_name = "GxVoodooFuzz" ;
	...
	cairo_set_font_size (ui->cr, 22.0);
...
static void controller_expose(gx_voodooUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_voodooUI *ui, int controller) {
...
static void check_value_changed(gx_voodooUI *ui, int i, float* value) {
...
static void check_is_active(gx_voodooUI *ui, int i, bool set) {
...
bool aligned(int x, int y, gx_controller *control, gx_voodooUI *ui) {
...
static void scroll_event(gx_voodooUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
...
static void set_key_value(gx_voodooUI *ui, int set_value) {
	float value = 0.0;
	for (int i=0;i<CONTROLS;i++) {
...
static void key_event(gx_voodooUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
		if (ui->controls[i].is_active) {
			value = min(ui->controls[i].adj.max_value,max(ui->controls[i].adj.min_value, 
...
static void set_previous_controller_active(gx_voodooUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
...
static void set_next_controller_active(gx_voodooUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
		if(ui->controls[i].is_active == true) {
			ui->controls[i].is_active = false;
			send_controller_event(ui, i);
...
static void motion_event(gx_voodooUI *ui, double start_value, XMotionEvent *mo) {
...
static void event_handler(gx_voodooUI *ui) {
	XEvent xev;
