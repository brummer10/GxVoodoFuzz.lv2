...
#define GTK_VOODOO_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_VOODOO_KNOB, GtkVoodooKnobPrivate))

G_DEFINE_TYPE(GtkVoodooKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkVoodooKnobPrivate GtkVoodooKnobPrivate;

struct _GtkVoodooKnobPrivate
{
	int knob_x;
	int knob_y;
	int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
	...
	GtkVoodooKnobPrivate *priv = GTK_VOODOO_KNOB_GET_PRIVATE(widget);
...
{
	g_assert(GTK_IS_VOODOO_KNOB(widget));
	GtkVoodooKnob *knob = GTK_VOODOO_KNOB(widget);
	GtkVoodooKnobPrivate *priv = GTK_VOODOO_KNOB_GET_PRIVATE(knob);
...
/****************************************************************
 ** init the GtkVoodooKnobClass
 */

static void gtk_knob_class_init (GtkVoodooKnobClass *klass)
{
...
//--------- connect the events with funktions
	...
	g_type_class_add_private(obj_class, sizeof (GtkVoodooKnobPrivate));
}
...
static void gtk_knob_init (GtkVoodooKnob *knob)
{
...
{
	GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_VOODOO_KNOB, NULL ));
